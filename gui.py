from sud_solv import valid, solve, find_empty
        ...
        for i in range(9):
            for j in range(9):
                if random.randint(1, 10) >= 5:
                    # plug in random number at random spot
                    board[i][j] = random.randint(1, 9)
                    if valid(board, (i, j), board[i][j]):
                        continue
                    else:
                        board[i][j] = 0
        # copies board without being modified after solve is called
            ...
            for j in range(4):
                if j % 3 == 0 and j != 0:  # vertical lines
                    pygame.draw.line(self.window, (0, 0, 0),
                                     ((j//3)*180, 0), ((j//3)*180, 540), 4)

                if i % 3 == 0 and i != 0:  # horizontal lines
                    pygame.draw.line(self.window, (0, 0, 0),
                                     (0, (i//3)*180), (540, (i//3)*180), 4)
        ...
        if len(keys) != 0:  # draws inputs that the user places on board but not their final value on that tile
            for value in keys:
                self.tiles[value[0]][value[1]].display(
                    keys[value], (21+(value[0]*60), (16+(value[1]*60))), (128, 128, 128))
                ...
                if board.board[selected[1]][selected[0]] == 0 and selected != (-1, -1):
                    ...
                    if event.key == pygame.K_9:
                        ...
                        if selected in keyDict:
                            ...
                            board.tiles[selected[1]][selected[0]
